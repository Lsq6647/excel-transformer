...
public class ExcelUtils {
    ...
    public static Map<Integer, List<String>> getExcelMap(MultipartFile mFile) throws Exception {
        return readExcelMap(mFile, null);
    }
    ...
    private static Map<Integer, List<String>> readExcelMap(MultipartFile mFile, File file) throws IOException {
        boolean fileNotExist = (file == null || !file.exists());
        if (mFile == null && fileNotExist) {
            return null;
        }
        ...
        Map<Integer, List<String>> map = readMap(book);
        book.close();
        in.close();
        return map;
    }
    ...
    private static Map<Integer, List<String>> readMap(Workbook book) {
        // 获取 Excel 文件第一个 Sheet 页面
        Sheet sheet = book.getSheetAt(0);
        return fetchExcel(sheet);
    }
    ...
    private static Map<Integer, List<String>> fetchExcel(Sheet sheet) {
...
//                        v = setCellValue(cell, o, headStyle);
    ...
    private static int setCellValue(Cell cell, Object o, CellStyle style) {
        ...
        if (o instanceof String) {
            String s = o.toString();
            if (isNumeric(s) && StringUtils.isEmpty(s)) {
